...
import com.humans.body.generator.retake.reshothumans.photoeditor.faceswapping.ads.config.models.NativeAdSettingsConfig
import com.humans.body.generator.retake.reshothumans.photoeditor.faceswapping.ads.config.models.NativeAdUnitConfig
import com.humans.body.generator.retake.reshothumans.photoeditor.faceswapping.ads.config.models.NativePlacement
import com.humans.body.generator.retake.reshothumans.photoeditor.faceswapping.ads.config.models.RemoteNativeConfigWrapper
...
class NativeAdConfigManager(
...
) : BaseRemoteConfigManager<RemoteNativeConfigWrapper>(firebaseRemoteConfig, "Config_v18") {
    private val configKey = "Config_v18"
    companion object {
    ...
    fun isNativeVisible(target: String, position: String): Boolean {
        ...
        return config.enabled == 1 && config.resolveUnit(target, position).enabled == 1
    }
    ...
    fun getNativeAdSize(target: String, position: String): Int {
        val config = getResolvedNativeConfig() ?: return getFallbackNativePlacementValue(target, position) ?: 1
        return config.resolveUnit(target, position).size ?: 2
    }

    private fun getFallbackNativePlacementValue(target: String, position: String): Int? {
        val normalizedPos = normalizePosition(position)
        val isHome = target in setOf("HomeFragmentScreen", "HomeScreen", "MainScreen")
        val isIntro = target == RemoteScreens.INTRO_SCREEN
        val isLanguage = target == RemoteScreens.LANGUAGE_SCREEN

        return when {
            isHome && normalizedPos == "center" -> 3
            isHome && normalizedPos == "bottom" -> 0
            isIntro && normalizedPos == "bottom" -> 4
            isIntro && normalizedPos == "fullscreen" -> 1
            isLanguage && normalizedPos == "bottom" -> 2
            else -> null
        }
    }

    private fun normalizePosition(position: String): String {
        return position.replace("_", "").lowercase()
    }
    ...
    private fun NativeAdColorConfig?.mergeWith(override: NativeAdColorConfig?): NativeAdColorConfig? {
        ...
        return NativeAdColorConfig(
            ...
            backgroundColorHex = override.normalized().backgroundColorHex ?: normalized().backgroundColorHex,
            ...
            headlineColorHex = override.normalized().headlineColorHex ?: normalized().headlineColorHex,
            bodyTextColorHex = override.normalized().bodyTextColorHex ?: normalized().bodyTextColorHex,
            ctaBackgroundColorHex = override.normalized().ctaBackgroundColorHex ?: normalized().ctaBackgroundColorHex,
            ctaTextColorHex = override.normalized().ctaTextColorHex ?: normalized().ctaTextColorHex,
            ctaCornerRadiusDp = override.normalized().ctaCornerRadiusDp ?: normalized().ctaCornerRadiusDp,
    ...
    private fun NativeAdColorConfig.normalized(): NativeAdColorConfig {
        return NativeAdColorConfig(
            mode = mode,
            backgroundColorHex = backgroundColorHex ?: darkBackgroundColorHex ?: lightBackgroundColorHex,
